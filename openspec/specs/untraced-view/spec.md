# untraced-view Specification

## Purpose
Reports the requests on this page that produced no span, and why — one cause per request, each
of them something the toolbar observed rather than inferred about the host's configuration.

This is the diagnostic no backend can produce. An un-instrumented request never arrives, so no
amount of querying would surface it, and the page cannot read its own request headers to check;
only the service worker sees it go out bare. That is what makes the capability the strongest
argument for tier 2, and it is also what makes its honesty load-bearing: with tier 2 off,
nothing carries a trace id the toolbar can see, so the naive count reads `M of M` on a page
that may be perfectly instrumented. The capability SHALL say it cannot tell rather than print
that number.
