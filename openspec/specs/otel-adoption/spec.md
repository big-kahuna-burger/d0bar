# otel-adoption Specification

## Purpose
Adopts the host page's own OpenTelemetry SDK when one is present, so that spans the application
already produces become the toolbar's tier 4 — and ships no SDK of its own to do it.

The distinction is the capability. d0bar attaches a read-only span processor to the provider the
host constructed; it does not install, configure, patch or wrap anything, and the build fails if
`@opentelemetry/` appears in the bundle. A toolbar that shipped its own SDK would be a second
tracer on the page competing with the first, which is the observer effect this project exists
not to have.

Everything below follows from that: detection is a feature test rather than a version check, a
provider that cannot be extended is reported rather than worked around, tier 1's own
measurements stay authoritative where the two disagree, and a span whose URL cannot be resolved
is left unjoined rather than guessed at.
